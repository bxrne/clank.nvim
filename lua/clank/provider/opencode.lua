...
---Build an opencode-style provider. OpenCode 1 and OpenCode 2 share the same
---`run` contract; only the installed binary differs.
---@param name string provider name registered in the registry
---@param binary string executable on $PATH (e.g. "opencode", "opencode2")
---@return clank.Provider
local function make(name, binary)
  ...
  function M.is_valid_model(model)
    ...
    -- opencode accepts any provider/model pair, optionally with a "#variant"
    -- suffix (OpenCode 2); validate the shape rather than enumerating every
    -- model across every provider.
  ...
  function M.send(opts, callbacks)
    ...
    local job = vim.system(cmd, {
      ...
      if result.code ~= 0 then
        ...
        if err == "" then
          err = ("%s exited with code %d"):format(binary, result.code)
        end
...
local opencode = make("opencode", "opencode")
